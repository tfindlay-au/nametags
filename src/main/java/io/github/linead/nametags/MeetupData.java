...
public class MeetupData {
    ...
    private static final String RSVP_URL = "https://api.meetup.com/2/rsvps?&sign=true&key={key}&event_id={event_id}&photo-host=public&page=100";

    private static final String MEMBERS_URL = "https://api.meetup.com/2/members?&key={key}&sign=true&photo-host=public&group_id={group_id}&page=100&only=id,joined,photo";
    ...
    public Map<String, Members.Member> getMembers(String groupId) {
        ...
        while(!StringUtils.isEmpty(members.getMeta().getNext())) {
            String nextUrl = members.getMeta().getNext() + "&key=" + getKey();
            responseEntity = restTemplate.getForEntity(nextUrl, Members.class, params);
